...
public class WarehouseObserverImp implements WarehouseObserver<CostumerArray> {
    ...
    public void update(CostumerArray array) {
        logger.log(Level.DEBUG, "Warehouse observer in use with array {}", array);
        Warehouse warehouse = Warehouse.getInstance();
        SummarizeActionImpl summarizeAction = new SummarizeActionImpl();
        MathParams params = summarizeAction.calculateAll(array).get();
        warehouse.put(array.getId(), params);
        logger.log(Level.DEBUG, "Warehouse result {}", warehouse.get(array.getId()));
    }
}
